...
void score_attack() {
	...
	sf::Http http;
	http.setHost("http://www.autistklassen.dk/");
	...
	sf::Text text_Score("Score: ", font_default);
	...
	sf::Text text_finish1("You finished with score: ", font_default);
	sf::Text text_finish2("Submit? <Y/N>", font_default);
	...
	sf::Clock hitClock;
	float mDelta;
	while(bzsflegacy::game::currentState == SCORE_ATTACK) {
			...
			switch(Event.type) {
				...
				case sf::Event::KeyPressed:
					if(Event.key.code == sf::Keyboard::Escape) {
						bzsflegacy::game::currentState = DIE;
					} else if(submitting && !typing_name) {
						if(Event.key.code == sf::Keyboard::N) {
							bzsflegacy::game::currentState = RESTART;
						} else if(Event.key.code == sf::Keyboard::Y) {
							typing_name = true;
							text_finish1.setString("Enter name:");
							text_finish2.setString(name);
						}
					}
					break;
				...
				case sf::Event::TextEntered:
					if(typing_name) {
						sf::Uint32 uc = Event.text.unicode;
						if(uc == 8) {
							if(name.length()!=0) {
							...
							if(name.compare("") == 0 || name.compare("y") == 0 || name.compare("Anonymousy") == 0) {
								name = "Anonymous";
							}

							sf::Http::Request request("LD26_scoreboard.php");
							...
							if(((char)uc)=='y' && name.length() == 0) {} else if(name.length() < 20) {
								std::stringstream ss;
								ss << (char)uc;
								name.append(ss.str());
								text_finish2.setString(name);
		...
		///////////////////////////////////////////////////////
				...
				if(buildup) {
					...
					if(elapsed < 700) {
						...
						if(health > 1) {
							...
							std::stringstream s;
							s << "You finished with score: " << score;
							text_finish1.setString(s.str());

							text_finish1.setOrigin(text_finish1.getLocalBounds().width/2, text_finish1.getLocalBounds().height/2);
							text_finish2.setOrigin(text_finish2.getLocalBounds().width/2, text_finish2.getLocalBounds().height/2);

							text_finish1.setPosition(640, 300);
							text_finish2.setPosition(640, 340);
			...
			if(dead) {
				...
				if(elapsed < 4000) {
					spr_fader.setColor(sf::Color(255, 255, 255, 255*(float(elapsed)/4000)));
					music_theme.setVolume(100.f - 100.f*(float(elapsed)/4000));
					music_rain.setVolume(20.f - 20.f*(float(elapsed)/4000));
				} else {
					spr_fader.setColor(sf::Color(255, 255, 255, 255));
					music_theme.stop();
					music_rain.stop();
		...
		score << "Time: " << floor(clock.getElapsedTime().asSeconds()*1000)/1000;
		text_Score.setString(score.str());
		if(!dead && begun && fadedin && !buildup) bzsflegacy::game::window->draw(text_Score);

		if(submitting) {
			bzsflegacy::game::window->draw(text_finish1);
			bzsflegacy::game::window->draw(text_finish2);
		}
